...
# ── Risk Score → Color Mapping (for human-readable reports) ───────────────────
RISK_COLORS = {
    "CRITICAL": "🔴",
    "HIGH":     "🟠",
    "MEDIUM":   "🟡",
...
def save_json_report(
    ...
    if flagged_count > 0:
        logger.info(f"  ⚠️  Risk distribution:")
...
def print_console_summary(results: List[FileScanResult]) -> None:
    ...
    if not flagged:
        print("\n✅ No PII found in any scanned files.\n")
        return

    print(f"\n{'='*60}")
    print(f"⚠️  PII DETECTED IN {len(flagged)} FILE(S)")
    print(f"{'='*60}")
    for result in sorted(flagged, key=lambda r: r.risk_score, reverse=True):
        icon = RISK_COLORS.get(result.risk_level, "")
        print(f"\n{icon} [{result.risk_level}] {result.file_name}")
